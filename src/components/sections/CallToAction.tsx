    ...
    >
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-[1.4fr_1fr] gap-x-12 gap-y-12 px-6 py-24 max-[860px]:grid-cols-1 max-[620px]:py-20">
        <div>
          ...
          <p className="mt-5 max-w-[44ch] text-[1.05rem] leading-relaxed text-muted">{desc}</p>
          <Link
            href={appHref}
            className="metallic-fill mt-8 inline-block rounded-full px-8 py-3.5 text-base font-semibold transition hover:-translate-y-px focus-visible:outline focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-[3px]"
          >
            {getStarted}
          </Link>
        </div>
        ...
        <div className="flex flex-col justify-center border-l border-white/10 pl-12 max-[860px]:border-l-0 max-[860px]:border-t max-[860px]:pl-0 max-[860px]:pt-10">
          ...
          <div className="mt-6 flex flex-col gap-3">
            <a
              ...
              className="group/link inline-flex w-fit items-center gap-2 text-sm text-fg transition-colors hover:text-accent"
            >
